 ...
 * 浏览相册- 存放图片的view
 */
public class BroImageView extends ImageView {
	...
	public BroImageView(Context context) {
		super(context);
	}

	public BroImageView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (width > BrowseAlbumActivity.screenWidth) {
			// 如果图宽大于屏宽，就不用水平居中
			center(false, true);
		} else if(height > BrowseAlbumActivity.screenHeight){
			center(true, false);
		}else{
			center(true, true);
		}
	...
	private void arithScaleRate() {
		float scaleWidth = BrowseAlbumActivity.screenWidth / (float) imageWidth;
		float scaleHeight = BrowseAlbumActivity.screenHeight / (float) imageHeight;
	...
	public void setImageBitmap(Bitmap bitmap) {
		...
		arithScaleRate();
		//缩放到屏幕大小
		zoomTo(scaleRate,BrowseAlbumActivity.screenWidth / 2f, BrowseAlbumActivity.screenHeight / 2f);
	...
	public void layoutToCenter()
	{
		...
		//空白区域宽高
		float fill_width = BrowseAlbumActivity.screenWidth - width;
		float fill_height = BrowseAlbumActivity.screenHeight - height;
	...
	protected float getValue(Matrix matrix, int whichValue) {
		...
		mMinZoom =( BrowseAlbumActivity.screenWidth/2f)/imageWidth;
