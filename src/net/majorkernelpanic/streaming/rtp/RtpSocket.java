	...
	public void run() {
		...
		try {
			...
			while (mBufferCommitted.tryAcquire(4,TimeUnit.SECONDS)) {
				...
				if(mCount++ > 30){
					byte[] mbyte = mPackets[mBufferOut].getData();
					byte[] newbuf = new byte[MTU-RTP_HEADER_LENGTH];
					System.arraycopy(mbyte, RTP_HEADER_LENGTH, newbuf, 0, MTU - RTP_HEADER_LENGTH);
					System.out.println(mbyte.length+"--"+newbuf.length);
					//AnalogvideoActivity.instance.avcCodec.setData(newbuf);
				}
